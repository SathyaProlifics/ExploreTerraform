...
#   monitoring_role_arn      = aws_iam_role.rds_monitoring.arn
  ...
  depends_on = [ aws_db_subnet_group.sub-grp ] # Ensure DB subnet group is created before the RDS instance
...
resource "aws_s3_bucket" "name" {
    bucket = "sathya-bucket-1234567890"
    
  
}

#Create read replica for master db
